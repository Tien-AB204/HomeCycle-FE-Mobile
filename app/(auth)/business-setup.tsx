...
export default function BusinessSetupScreen() {
  ...
  const handleNextToForm = () => {
    if (!model) {
      Platform.OS === "web"
        ? window.alert("Vui lòng chọn mô hình kinh doanh!")
  ...
  const handleSubmit = async () => {
    ...
    if (fullName.trim().toLowerCase() !== identityName.trim().toLowerCase()) {
      Platform.OS === "web"
        ? window.alert(
            "Lỗi: 'Họ và tên người đại diện' bắt buộc phải giống hệt với 'Họ tên trên CCCD'!",
          )
        : alert(
            "Lỗi: 'Họ và tên người đại diện' bắt buộc phải giống hệt với 'Họ tên trên CCCD'!",
          );
    ...
    if (!businessLicense || !frontImage || !backImage) {
      Platform.OS === "web"
        ? window.alert(
            "Vui lòng tải lên đầy đủ Giấy chứng nhận ĐKKD và CCCD (2 mặt)!",
          )
        : alert(
            "Vui lòng tải lên đầy đủ Giấy chứng nhận ĐKKD và CCCD (2 mặt)!",
          );
    ...
    try {
      ...
      if (!token || token === "null") {
        Platform.OS === "web"
          ? window.alert(
              "Lỗi: Không tìm thấy Token xác thực. Vui lòng đăng nhập lại!",
            )
          : alert(
              "Lỗi: Không tìm thấy Token xác thực. Vui lòng đăng nhập lại!",
            );
      ...
      const errorMsg =
        ...
        "Mạng chậm, upload ảnh thất bại hoặc lỗi server!";
