        ...
        <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.avatarContainer}>
            <TouchableOpacity style={styles.avatarBox} onPress={pickAvatar}>
              {avatarUri ? (
                <Image source={{ uri: avatarUri as string }} style={{ width: '100%', height: '100%', borderRadius: 24 }} />
              ) : (
          ...
          <View style={styles.inputContainer}>
            <TextInput style={[styles.input, Platform.OS === "web" && ({ outlineStyle: "none" } as any)]} placeholder="Nhập họ và tên..." placeholderTextColor={COLORS.textLight} value={fullName} onChangeText={setFullName} />
          </View>

          <Text style={styles.fieldLabel}>Username *</Text>
          <View style={styles.inputContainer}>
            <TextInput style={[styles.input, Platform.OS === "web" && ({ outlineStyle: "none" } as any)]} placeholder="username_cua_ban" placeholderTextColor={COLORS.textLight} autoCapitalize="none" value={username} onChangeText={setUsername} />
          </View>

          <Text style={styles.fieldLabel}>Số điện thoại *</Text>
          <View style={styles.inputContainer}>
            <TextInput style={[styles.input, Platform.OS === "web" && ({ outlineStyle: "none" } as any)]} placeholder="Nhập số điện thoại..." placeholderTextColor={COLORS.textLight} keyboardType="phone-pad" value={phone} onChangeText={setPhone} />
          </View>
          <View style={styles.privacyNoteContainer}>
            <Ionicons name="lock-closed-outline" size={14} color={COLORS.textLight} />
            <Text style={styles.privacyNoteText}>Số điện thoại của bạn sẽ được bảo mật.</Text>
          </View>
