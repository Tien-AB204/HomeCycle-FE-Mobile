  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.list}>
        {MOCK_SCHEDULES.filter(s => activeTab === 'all' || s.type === activeTab).map((item) => (
          <View key={item.id} style={styles.card}>
            ...
            <Text style={styles.partnerName}>
              <Ionicons name="person-outline" size={14} /> Đối tác: {item.partner}
            </Text>
            ...
            <View style={styles.actions}>
              ...
              <TouchableOpacity style={[styles.outlineBtn, { borderColor: COLORS.error }]}>
                <Text style={[styles.outlineBtnText, { color: COLORS.error }]}>Hủy lịch</Text>
              </TouchableOpacity>
            </View>
          </View>
        ))}
...
const styles = StyleSheet.create({
  ...
  productName: { fontSize: 16, fontWeight: "bold", color: "#1E293B", marginBottom: 6 },
  partnerName: { fontSize: 14, color: "#475569", marginBottom: 12 },
  infoRow: { flexDirection: "row", gap: 8, marginBottom: 8, alignItems: "center" },
  infoText: { fontSize: 13, color: "#475569", flex: 1 },
  actions: { flexDirection: "row", gap: 12, marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderColor: "#F1F5F9" },
  outlineBtn: { flex: 1, paddingVertical: 10, borderRadius: 8, borderWidth: 1, borderColor: "#CBD5E1", alignItems: "center" },
