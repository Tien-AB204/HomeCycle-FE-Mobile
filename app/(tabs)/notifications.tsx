...
const MOCK_NOTIFS = [
  {
    ...
    message: "Lịch kiểm định đơn ORD-998877 đã trôi qua 1 giờ. Vui lòng cập nhật trạng thái nếu không hệ thống sẽ tự động hủy đơn.",
    time: "10 phút trước",
    unread: true,
  ...
  },
  {
    ...
    message: "Công ty ABC vừa gửi đề nghị mua Tủ lạnh Panasonic của bạn với giá 2.500.000đ.",
    time: "2 giờ trước",
    unread: true,
  ...
  },
  {
    ...
    message: "Tin đăng 'Sofa góc L' của bạn đã được kiểm duyệt và đang hiển thị công khai.",
    time: "1 ngày trước",
    unread: false,
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.list}>
        {MOCK_NOTIFS.map(notif => (
          <TouchableOpacity key={notif.id} style={[styles.notifItem, notif.unread && styles.unreadItem]}>
            <View style={[styles.iconBox, { backgroundColor: notif.bg }]}>
              <Ionicons name={notif.icon as any} size={24} color={notif.color} />
            </View>
            <View style={styles.content}>
              <Text style={[styles.title, notif.unread && { fontWeight: "bold", color: "#0F172A" }]}>{notif.title}</Text>
...
const styles = StyleSheet.create({
  ...
  dot: { width: 10, height: 10, borderRadius: 5, backgroundColor: COLORS.error, marginTop: 6, marginLeft: 8 }
});