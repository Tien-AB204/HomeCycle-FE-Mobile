...
export default function PostsScreen() {
  ...
  const handleClosePost = (postId: string) => {
    ...
    if (Platform.OS === "web") {
      if (window.confirm("Bạn có chắc chắn muốn đóng (kết thúc giao dịch) tin đăng này không?")) executeClose();
    } else {
      Alert.alert(
        "Đóng bài đăng",
  ...
  const translateStatus = (status: string) => {
    switch (status) {
      ...
      case "Deleted": return { text: "Đã xóa", color: "#EF4444", bg: "#FEE2E2" };
      case "Closed": return { text: "Đã đóng", color: "#64748B", bg: "#E2E8F0" };
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={[styles.mobileWrapper, isWeb ? { width: 480, alignSelf: "center" } : null]}>
          ...
          <View style={styles.unauthContainer}>
            <Ionicons name="document-text-outline" size={80} color="#CBD5E1" style={{ marginBottom: 16 }} />
            <Text style={styles.unauthTitle}>Bạn chưa đăng nhập</Text>
            <Text style={styles.unauthDesc}>
              Hãy đăng nhập để quản lý bài đăng, theo dõi trạng thái giao dịch và đăng tin mới nhé!
            </Text>
            <TouchableOpacity style={styles.loginBtn} onPress={() => router.push("/(auth)/login?returnUrl=/(tabs)/posts")}>
  ...
  const activePosts = validPosts.filter((p) => p.status === "Active" || p.status === "Pending");
    ...
    return (
      ...
      >
        ...
        <View style={styles.cardContent}>
          ...
          <Text style={styles.descText} numberOfLines={2}>{post.description}</Text>
          ...
          <View style={styles.tagGrid}>
            <View style={[styles.tag, { backgroundColor: statusObj.bg }]}>
              <Text style={[styles.tagText, { color: statusObj.color, fontWeight: "bold" }]}>{statusObj.text}</Text>
            </View>
            <View style={styles.tag}><Text style={styles.tagText}>Loại: {post.postType}</Text></View>
            <View style={styles.tag}><Text style={styles.tagText}>SL: {post.remainingQuantity} / {post.quantity}</Text></View>
            {post.deliveryMethod && <View style={styles.tag}><Text style={styles.tagText}>Giao hàng: {post.deliveryMethod}</Text></View>}
            {post.priorityLevel && <View style={styles.tag}><Text style={styles.tagText}>Ưu tiên: {post.priorityLevel}</Text></View>}
          </View>
            ...
            <View style={{ alignItems: "flex-end" }}>
              ...
              <View style={styles.actionButtons}>
                <TouchableOpacity
                  ...
                  onPress={(e) => {
                    e.stopPropagation();
                    router.push({ pathname: "/posts/post-form", params: { editId: post.postId, postType: post.postType } });
                  }}
                >
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={[styles.mobileWrapper, isWeb ? { width: 480, alignSelf: "center" } : null]}>
        ...
        <View style={styles.tabContainer}>
          ...
          <TouchableOpacity
            style={[styles.tabBtn, activeTab === "closed" ? styles.tabBtnActive : null]}
            onPress={() => setActiveTab("closed")}
          >
            <Text style={[styles.tabText, activeTab === "closed" ? styles.tabTextActive : null]}>
            ...
            {activeTab === "active" ? (
              activePosts.length > 0 ? (
                ...
                <Text style={styles.emptyText}>
                  {userRole === "personal" ? "Chưa có tin đăng nào đang hoạt động." : "Chưa có tin thu mua nào đang hoạt động."}
                </Text>
              )
...
const styles = StyleSheet.create({
  ...
  unauthContainer: { flex: 1, justifyContent: "center", alignItems: "center", padding: 24, backgroundColor: COLORS.white },
  unauthTitle: { fontSize: 20, fontWeight: "bold", color: COLORS.text, marginBottom: 12 },
  unauthDesc: { fontSize: 14, color: COLORS.textLight, textAlign: "center", marginBottom: 32, lineHeight: 22 },
  loginBtn: { backgroundColor: COLORS.primary, paddingHorizontal: 32, paddingVertical: 14, borderRadius: 8 },
  ...
  card: { flexDirection: "row", backgroundColor: COLORS.white, borderRadius: 12, padding: 12, marginBottom: 16, borderWidth: 1, borderColor: COLORS.border, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 2 },
  ...
  loadMoreBtn: { padding: 12, alignItems: "center", backgroundColor: "#E2E8F0", borderRadius: 8, marginVertical: 10 },
