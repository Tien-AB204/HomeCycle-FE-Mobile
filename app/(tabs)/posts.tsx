...
export default function PostsScreen() {
  ...
  const mockSellingPosts = [
    { id: 1, title: 'Tủ lạnh Samsung Inverter 236L', price: '3.500.000 đ', image: 'https://images.unsplash.com/photo-1584568694244-14fbdf83bd30?q=80&w=200&auto=format&fit=crop', status: 'Hoạt động tốt', expires: '30 ngày nữa' },
    { id: 2, title: 'Sofa góc bọc da cao cấp xám nhạt', price: '2.100.000 đ', image: 'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?q=80&w=200&auto=format&fit=crop', status: 'Hư nhẹ', expires: '15 ngày nữa' },
  ];
  ...
  const renderCard = (post: any, isBuying: boolean) => (
    <View key={post.id} style={styles.card}>
      ...
      <View style={styles.cardContent}>
        ...
        <Text style={styles.cardPrice}>{isBuying ? post.priceRange : post.price}</Text>
        ...
        <View style={styles.cardActions}>
          <Text style={[styles.statsText, post.expires === 'Đã đóng' ? { color: COLORS.error, fontWeight: 'bold' } : null]}>
          ...
          <View style={styles.actionButtons}>
            ...
            <TouchableOpacity style={styles.iconBtn}><Ionicons name={isBuying ? "lock-closed-outline" : "eye-off-outline"} size={18} color={COLORS.error} /></TouchableOpacity>
          </View>
        </View>
      </View>
    </View>
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={[styles.mobileWrapper, isWeb ? { width: 480, alignSelf: 'center' } : null]}>
        ...
        <View style={styles.tabContainer}>
          {userRole === 'personal' ? (
            <>
              <TouchableOpacity style={[styles.tabBtn, activePersonalTab === 'active' ? styles.tabBtnActive : null]} onPress={() => setActivePersonalTab('active')}>
                <Text style={[styles.tabText, activePersonalTab === 'active' ? styles.tabTextActive : null]}>Đang hiển thị (2)</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.tabBtn, activePersonalTab === 'hidden' ? styles.tabBtnActive : null]} onPress={() => setActivePersonalTab('hidden')}>
                <Text style={[styles.tabText, activePersonalTab === 'hidden' ? styles.tabTextActive : null]}>Đã ẩn / Đã bán (0)</Text>
              </TouchableOpacity>
            </>
          ) : (
            <>
              <TouchableOpacity style={[styles.tabBtn, activeBusinessTab === 'buying' ? styles.tabBtnActive : null]} onPress={() => setActiveBusinessTab('buying')}>
                <Text style={[styles.tabText, activeBusinessTab === 'buying' ? styles.tabTextActive : null]}>Tin Thu Mua (2)</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.tabBtn, activeBusinessTab === 'requests' ? styles.tabBtnActive : null]} onPress={() => setActiveBusinessTab('requests')}>
                <Text style={[styles.tabText, activeBusinessTab === 'requests' ? styles.tabTextActive : null]}>Yêu cầu chào hàng (5)</Text>
              </TouchableOpacity>
            </>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  fabButton: { 
    ...
    ...(Platform.OS === 'web' ? { boxShadow: '0px 4px 6px rgba(0,0,0,0.3)' } as any : { shadowColor: COLORS.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 6, elevation: 6 })
  },
});