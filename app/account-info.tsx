...
export default function AccountInfoScreen() {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (profileChanged) {
        apiTasks.push(apiClient.put('/personals/me/profile', { username, fullName, phoneNumber, address }));
      }
      ...
      if (newAvatarFile) {
        ...
        apiTasks.push(apiClient.patch('/personals/me/avatar', formData, { timeout: 60000 }));
      }
      ...
      if (identityChanged) {
        ...
        apiTasks.push(apiClient.put('/personals/me/identity', formData, { timeout: 60000 }));
      }
      ...
      if (bankChanged) {
        apiTasks.push(apiClient.put('/personals/me/bank', { bankCode: bankCode || 'VNBANK', bankName, accountNumber, accountName }));
      }
  ...
  const displayAvatar = newAvatarFile?.uri ? { uri: newAvatarFile.uri } : ((avatarUrl && !imageError) ? { uri: avatarUrl } : { uri: defaultAvatar });
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.inputContainer, { height: 80, alignItems: 'flex-start', paddingTop: 12 }]}>
            <TextInput style={styles.input} value={address} onChangeText={setAddress} multiline placeholder="Nhập địa chỉ..." />
          </View>
          ...
          <View style={styles.cccdRow}>
            <TouchableOpacity style={styles.cccdBox} onPress={() => pickImage('front')}>
              {(frontImage?.uri || sanitize(user?.frontIDCardImage)) ? (
                <Image source={{ uri: frontImage?.uri || sanitize(user?.frontIDCardImage) }} style={{ width: '100%', height: '100%', borderRadius: 12 }} resizeMode="cover" />
              ) : (
                <>
            ...
            <TouchableOpacity style={styles.cccdBox} onPress={() => pickImage('back')}>
              {(backImage?.uri || sanitize(user?.backIDCardImage)) ? (
                <Image source={{ uri: backImage?.uri || sanitize(user?.backIDCardImage) }} style={{ width: '100%', height: '100%', borderRadius: 12 }} resizeMode="cover" />
              ) : (
                <>
                  <Ionicons name="camera-outline" size={24} color={COLORS.primary} style={{marginBottom: 4}}/>
