...
const normalizeComparableAddress = (value: string) =>
  value
    .normalize("NFC")
    .trim()
    .replace(/\s*,\s*/g, ",")
...
const areAddressesExactlySame = (first: string, second: string) => {
  const normalizedFirst = normalizeComparableAddress(first);
  const normalizedSecond = normalizeComparableAddress(second);
...
export default function AgreementFormScreen() {
  ...
  const addressResolutionInFlightRef = useRef(false);
  const inspectionAddressResolutionRef = useRef<ResolvedVietnameseAddress | null>(null);
  const pickupAddressResolutionRef = useRef<ResolvedVietnameseAddress | null>(null);
  const deliveryAddressResolutionRef = useRef<ResolvedVietnameseAddress | null>(null);
  ...
  const pickupAddressForComparison =
    deliveryMethod === "GHN"
      ? composeAddress(sender)
      : pickupAddress;
  const deliveryAddressForComparison =
    deliveryMethod === "GHN"
      ? composeAddress(receiver)
      : deliveryAddress;
  ...
  const resolveRequiredAddress = async (
    label: string,
    rawValue: string,
    resolutionRef: React.MutableRefObject<ResolvedVietnameseAddress | null>,
    ...
    if (
      current &&
      current.formattedAddress.trim().toLocaleLowerCase("vi-VN") ===
        trimmed.toLocaleLowerCase("vi-VN")
    ) {
      return current;
    }

    try {
      const resolved = await resolveVietnameseAddress(trimmed);
      if (!resolved) {
        setNotice({
          type: "error",
          message: `Không xác định được Tỉnh/Thành và Phường/Xã của ${label}. Vui lòng nhập địa chỉ đầy đủ hoặc dùng nút chọn địa chỉ.`,
        });
        return null;
      }

      resolutionRef.current = resolved;
      applyFormattedValue(resolved.formattedAddress);
      return resolved;
    } catch {
      setNotice({
        type: "error",
        message: `Không thể xác minh Tỉnh/Thành và Phường/Xã của ${label} lúc này. Vui lòng thử lại hoặc dùng nút chọn địa chỉ.`,
      });
      return null;
    }
  };
  ...
  const validateGhnForm = () => {
    ...
    if (sameAddressError) return sameAddressError;
  ...
  return <SafeAreaView style={styles.safeArea}><Header title={isEditing ? "Chỉnh sửa hợp đồng" : "Thiết lập hợp đồng"} showBack /><KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.flex}><ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
    ...
    {deliveryMethod === "GHN" ? <View style={styles.configCard}><View style={styles.ghnHeader}><Ionicons name="cube-outline" size={22} color={COLORS.primary} /><Text style={styles.ghnTitle}>Thông tin giao hàng nhanh (GHN)</Text></View>{isLoadingGhnInfo ? <View style={styles.ghnLoadingBox}><ActivityIndicator size="small" color={COLORS.primary} /><Text style={styles.ghnLoadingText}>Đang tự động lấy thông tin kiện hàng...</Text></View> : <><GhnPartyFields title="Thông tin người gửi" value={sender} isExpanded={senderExpanded} onToggle={() => setSenderExpanded(!senderExpanded)} provinces={provinces} districts={senderDistricts} wards={senderWards} loadingDistricts={loadingSenderDistricts} loadingWards={loadingSenderWards} onChange={updateSender} onSelectProvince={selectSenderProvince} onSelectDistrict={selectSenderDistrict} onSelectWard={(ward) => updateSender({ ward })} /><GhnPartyFields title="Thông tin người nhận" value={receiver} isExpanded={receiverExpanded} onToggle={() => setReceiverExpanded(!receiverExpanded)} provinces={provinces} districts={receiverDistricts} wards={receiverWards} loadingDistricts={loadingReceiverDistricts} loadingWards={loadingReceiverWards} onChange={updateReceiver} onSelectProvince={selectReceiverProvince} onSelectDistrict={selectReceiverDistrict} onSelectWard={(ward) => updateReceiver({ ward })} />{sameAddressError ? <Text accessibilityRole="alert" style={styles.numericError}>{sameAddressError}</Text> : null}
        ...
        </View> : null}</>}</View> : <><Text style={styles.subLabel}>Thông tin người bán</Text><View style={styles.inputContainer}><Text style={styles.inputLabel}>Họ và tên người bán</Text><TextInput value={sellerInfo.fullName} onChangeText={(fullName) => setSellerInfo((current) => ({ ...current, fullName: capitalizeWordInitials(fullName) }))} placeholder="Nhập họ và tên" placeholderTextColor="#547B7D" autoCapitalize="words" autoCorrect={false} style={styles.input} editable={!isProcessing} /></View><View style={styles.inputContainer}><Text style={styles.inputLabel}>Số điện thoại người bán</Text><TextInput value={sellerInfo.phone} onChangeText={(phone) => setSellerInfo((current) => ({ ...current, phone: phone.replace(/[^0-9+]/g, "") }))} placeholder="Nhập số điện thoại" placeholderTextColor="#547B7D" keyboardType="phone-pad" style={styles.input} editable={!isProcessing} /></View><View style={styles.addressFieldBlock}><Text style={styles.inputLabel}>Địa chỉ lấy hàng (Người bán) <Text style={{ color: COLORS.error }}>*</Text></Text><AgreementAddressField value={pickupAddress} onChange={(value, selection) => { setPickupAddress(value); pickupAddressResolutionRef.current = selection ? { ...selection } : null; setSellerInfo((current) => selection ? { ...current, streetAddress: selection.streetAddress || value, ward: selection.wardName || "", city: selection.provinceName || "" } : { ...current, streetAddress: value, ward: "", city: "" }); }} onClear={() => { setPickupAddress(""); pickupAddressResolutionRef.current = null; setSellerInfo((current) => ({ ...current, streetAddress: "", ward: "", city: "" })); }} placeholder="Nhập địa chỉ lấy hàng..." disabled={isProcessing} /></View><View style={styles.addressFieldBlock}><Text style={styles.inputLabel}>Địa chỉ nhận hàng (Người mua) <Text style={{ color: COLORS.error }}>*</Text></Text><AgreementAddressField value={deliveryAddress} onChange={(value, selection) => { setDeliveryAddress(value); deliveryAddressResolutionRef.current = selection ? { ...selection } : null; }} onClear={() => { setDeliveryAddress(""); deliveryAddressResolutionRef.current = null; }} placeholder="Nhập địa chỉ nhận hàng..." disabled={isProcessing} />{sameAddressError ? <Text accessibilityRole="alert" style={styles.numericError}>{sameAddressError}</Text> : null}</View></>}
