...
import {
  ...
  ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View, Modal, FlatList,
} from "react-native";
...
const SPACE_USAGE_OPTIONS = [
  { label: "Phòng khách", value: "Living_room" }, { label: "Nhà bếp", value: "Kitchen" },
  { label: "Phòng ngủ", value: "Bedroom" }, { label: "Phòng tắm", value: "Bathroom" },
  { label: "Phòng giặt", value: "Laundry_room" }, { label: "Ban công", value: "Balcony" },
  { label: "Garage", value: "Garage" }, { label: "Nhà vệ sinh", value: "Restroom" },
];
const DAMAGE_LEVEL_OPTIONS = [
  { label: "Không hư hại (Như mới)", value: "None" }, { label: "Trầy xước ngoại hình", value: "Cosmetic_Damage" },
  { label: "Hư hỏng nhẹ", value: "Minor_Damage" }, { label: "Hư hỏng vừa", value: "Moderate_Damage" },
  { label: "Hư hỏng nặng", value: "Severe_Damage" }, { label: "Mất hoàn toàn chức năng", value: "Total_Loss" },
];
const FUNC_STATUS_OPTIONS = [
  { label: "Hoạt động hoàn hảo", value: "FullyFunctional" }, { label: "Hoạt động một phần", value: "PartiallyFunctional" },
  { label: "Không hoạt động", value: "NonFunctional" },
];
const DELIVERY_OPTIONS = [
  { label: "Chưa xác định", value: "Unknown" }, { label: "Giao hàng (GHN)", value: "GhnDelivery" },
  { label: "Người bán tự giao", value: "SellerDelivers" }, { label: "Người mua đến lấy", value: "BuyerPickUp" },
];
const PRIORITY_OPTIONS = [
  { label: "Bình thường (Medium)", value: "Medium" }, { label: "Bán gấp (High)", value: "High" }, { label: "Thấp (Low)", value: "Low" },
];
...
export default function PostFormScreen() {
  ...
  // Dùng useRef thay vì useState để lưu function an toàn tuyệt đối
    ...
    const fetchMasterData = async () => {
      try {
        const [catRes, ptRes, brandRes] = await Promise.all([
          postApi.getAllCategories(), postApi.getAllProductTypes(), postApi.getAllBrands(),
        ]);
        if (catRes?.data?.items) setCategories(catRes.data.items.map((c: any) => ({ label: c.categoryName, value: c.categoryId })));
      ...
      } catch (error) { console.error("Lỗi lấy Master Data:", error); }
    };
    fetchMasterData();
  }, []);

  // 2. NẾU LÀ CHẾ ĐỘ SỬA -> TẢI DỮ LIỆU CŨ VÀ ĐỔ VÀO FORM
  useEffect(() => {
    ...
    const loadOldData = async () => {
      ...
      } catch (error) { console.error("Lỗi tải dữ liệu cũ:", error); } 
      finally { setIsFetchingOldData(false); }
    };
    loadOldData();
  }, [editId]);
  ...
  useEffect(() => {
    const fetchDynamicSchema = async () => {
      ...
      } catch (error) { console.error("Lỗi lấy Schema EAV:", error); } 
      finally { setIsLoadingSchema(false); }
    };
    fetchDynamicSchema();
  }, [selectedProductType, oldEavData]); // <-- Lưu ý oldEavData ở đây

  const openDimensionsModal = () => { setTempLength(length); setTempWidth(width); setTempHeight(height); setShowDimensionsModal(true); };
  const confirmDimensions = () => { setLength(tempLength); setWidth(tempWidth); setHeight(tempHeight); setShowDimensionsModal(false); };
  const clearDimensions = () => { setLength(""); setWidth(""); setHeight(""); };
  const displayDimensions = (length && width && height) ? `${length} x ${width} x ${height} cm` : "";

  const openAddressModal = () => { setTempCity(city); setTempWard(ward); setTempStreet(streetAddress); setShowAddressModal(true); };
  const confirmAddress = () => { setCity(tempCity); setWard(tempWard); setStreetAddress(tempStreet); setShowAddressModal(false); };
  const clearAddress = () => { setCity(""); setWard(""); setStreetAddress(""); };
  const displayAddress = [streetAddress, ward, city].filter(Boolean).join(", ");

  const pickImages = async () => {
    let result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ImagePicker.MediaTypeOptions.Images, allowsMultipleSelection: true, selectionLimit: 10 - images.length, quality: 0.8,
    });
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (selectedCategory) formData.append("Product.CategoryId", selectedCategory);
      if (selectedProductType) formData.append("Product.ProductTypeId", selectedProductType);
      if (brandId) formData.append("Product.BrandId", brandId);
      if (modelNumber) formData.append("Product.ModelNumber", modelNumber);
      if (originalPrice) formData.append("Product.OriginalPrice", originalPrice);
      if (detailDescription) formData.append("Product.DetailDescription", detailDescription);
      
      if (weight) formData.append("Product.Weight", weight);
      if (usageDuration) formData.append("Product.UsageDuration", usageDuration);
      if (spaceUsage) formData.append("Product.SpaceUsage", spaceUsage);
      if (functionalityStatus) formData.append("Product.FunctionalityStatus", functionalityStatus);
      if (damageLevel) formData.append("Product.DamageLevel", damageLevel);

      if (length) formData.append("Product.Length", length);
      if (width) formData.append("Product.Width", width);
      if (height) formData.append("Product.Height", height);

      // ================= ĐÓNG GÓI PAYLOAD EAV MỚI (BẮT CHƯỚC SWAGGER JSON) =================
      eavAttributes.forEach((attr) => {
        ...
        // Bỏ qua nếu không điền gì và isRequired = false
        if (!hasOption && !hasText && !hasNumber && !hasBool && !attr.isRequired) return;

        // Tạo ra Object y hệt như cái ô nhập liệu trên Swagger của bạn
        const item: any = {
          attributeId: attr.attributeId
        };

        // Quy tắc: Chỉ gửi 1 trong 4 giá trị
        if (hasOption) {
          item.optionId = attr.selectedOptionId;
        } else if (hasBool) {
          item.valueBoolean = attr.valueBoolean; // Gửi thẳng kiểu boolean
        } else if (hasNumber) {
          item.valueNumber = Number(attr.valueNumber); // Ép kiểu số
        } else if (hasText) {
          item.valueText = attr.valueText;
        }

        // BÍ QUYẾT: Ép object thành chuỗi JSON và gửi đi cùng 1 tên key.
        // Đây chính là cách Swagger UI đẩy mảng object vào multipart/form-data!
        formData.append("Product.AttributeValues", JSON.stringify(item));
      });

      // Xử lý ảnh (Kể cả ảnh cũ)
      await Promise.all(
        images.map(async (imageUri, index) => {
          ...
          if (!filename.includes(".")) filename = `${filename}.jpg`; // Fix lỗi file Web không có đuôi
          ...
          if (Platform.OS === "web") {
            ...
            formData.append("Medias", { uri: Platform.OS === "ios" ? imageUri.replace("file://", "") : imageUri, name: filename, type: type } as any);
          }
        })
      );

      if (isEditMode) {
        if (urlPostType === "Sell" || userRole === "personal") {
    ...
    <SafeAreaView style={styles.safeArea}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>{isEditMode ? "Sửa tin đăng" : "Đăng tin mới"}</Text>
          <TouchableOpacity onPress={handlePublish} disabled={isLoading}>
            {isLoading ? <ActivityIndicator color={COLORS.primary} /> : <Text style={styles.publishButtonText}>{isEditMode ? "Cập nhật" : "Đăng"}</Text>}
          </TouchableOpacity>
        </View>
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.cardSection}>
            <Text style={styles.label}>Hình ảnh sản phẩm <Text style={styles.required}>* (2-10 ảnh)</Text></Text>
          ...
          <View style={styles.cardSection}>
            ...
            <View style={styles.inputContainer}>
              <TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} placeholder="Nhập tiêu đề sản phẩm..." placeholderTextColor="#94A3B8" value={productName} onChangeText={setProductName} />
            </View>
            ...
            <View style={[styles.inputContainer, { height: 80, alignItems: "flex-start", paddingTop: 12 }]}>
              <TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} multiline placeholder="VD: Cần pass gấp tủ lạnh vì chuyển trọ..." placeholderTextColor="#94A3B8" value={description} onChangeText={setDescription} />
            </View>
            ...
            <View style={styles.row}>
              <View style={{ flex: 1 }}>
                ...
                <TouchableOpacity style={styles.inputContainer} onPress={() => openSelect("Chọn Phân loại", categories, setSelectedCategory)}>
                  <Text style={selectedCategory ? styles.inputText : styles.placeholderText} numberOfLines={1}>{selectedCategory ? getLabel(selectedCategory, categories) : "Chọn..."}</Text>
              ...
              <View style={{ flex: 1 }}>
                ...
                <TouchableOpacity style={styles.inputContainer} onPress={() => openSelect("Chọn Loại Sản Phẩm", filteredProductTypes, setSelectedProductType)}>
                  <Text style={selectedProductType ? styles.inputText : styles.placeholderText} numberOfLines={1}>{selectedProductType ? getLabel(selectedProductType, filteredProductTypes) : "Chọn..."}</Text>
                  <Ionicons name="chevron-down" size={20} color="#94A3B8" />
                </TouchableOpacity>
              </View>
            </View>
            ...
            <View style={styles.row}>
              <View style={{ flex: 1 }}>
                ...
                <TouchableOpacity style={styles.inputContainer} onPress={() => openSelect("Chọn Thương hiệu", brands, setBrandId)}>
                  <Text style={brandId ? styles.inputText : styles.placeholderText} numberOfLines={1}>{brandId ? getLabel(brandId, brands) : "Chọn..."}</Text>
                  <Ionicons name="chevron-down" size={20} color="#94A3B8" />
                </TouchableOpacity>
              </View>
              <View style={{ flex: 1 }}>
                ...
                <View style={styles.inputContainer}>
                  <TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} placeholder="VD: RT38K5982BS" placeholderTextColor="#94A3B8" value={modelNumber} onChangeText={setModelNumber} />
                </View>
              </View>
            </View>
            ...
            <View style={[styles.inputContainer, { height: 100, alignItems: "flex-start", paddingTop: 12 }]}>
              <TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} multiline placeholder="Mô tả tình trạng, đặc điểm nổi bật của máy..." placeholderTextColor="#94A3B8" value={detailDescription} onChangeText={setDetailDescription} />
            </View>
          </View>
                  ...
                  <View key={attr.attributeId} style={styles.rawBlock}>
                    ...
                    {isOptionAllowed && (
                      <TouchableOpacity 
                        ...
                        onPress={() => openSelect(`Chọn ${attr.attributeName}`, optionList, (val) => updateEavValue(index, 'selectedOptionId', val))}
                      >
          ...
          <View style={styles.cardSection}>
            ...
            <View style={styles.row}>
              <View style={{ flex: 1 }}><Text style={styles.label}>Kích thước (DxRxC)</Text><TouchableOpacity style={styles.inputContainer} onPress={openDimensionsModal}><Text style={displayDimensions ? styles.inputText : styles.placeholderText} numberOfLines={1}>{displayDimensions || "VD: 120 x 60 x 80 cm"}</Text>{displayDimensions && <TouchableOpacity onPress={clearDimensions} style={styles.clearIcon}><Ionicons name="close-circle" size={20} color="#94A3B8" /></TouchableOpacity>}</TouchableOpacity></View>
              <View style={{ flex: 1 }}><Text style={styles.label}>Cân nặng (kg)</Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} keyboardType="numeric" placeholder="VD: 15" placeholderTextColor="#94A3B8" value={weight} onChangeText={setWeight} /></View></View>
            </View>
            <View style={styles.row}>
              <View style={{ flex: 1 }}><Text style={styles.label}>Không gian dùng</Text><TouchableOpacity style={styles.inputContainer} onPress={() => openSelect("Chọn Không Gian Dùng", SPACE_USAGE_OPTIONS, setSpaceUsage)}><Text style={spaceUsage ? styles.inputText : styles.placeholderText} numberOfLines={1}>{spaceUsage ? getLabel(spaceUsage, SPACE_USAGE_OPTIONS) : "Chọn..."}</Text>{spaceUsage ? <TouchableOpacity onPress={() => setSpaceUsage("")} style={styles.clearIcon}><Ionicons name="close-circle" size={20} color="#94A3B8" /></TouchableOpacity> : <Ionicons name="chevron-down" size={20} color="#94A3B8" />}</TouchableOpacity></View>
              <View style={{ flex: 1 }}><Text style={styles.label}>Mức độ hư hại</Text><TouchableOpacity style={styles.inputContainer} onPress={() => openSelect("Mức Độ Hư Hại", DAMAGE_LEVEL_OPTIONS, setDamageLevel)}><Text style={damageLevel ? styles.inputText : styles.placeholderText} numberOfLines={1}>{damageLevel ? getLabel(damageLevel, DAMAGE_LEVEL_OPTIONS) : "Chọn..."}</Text>{damageLevel ? <TouchableOpacity onPress={() => setDamageLevel("")} style={styles.clearIcon}><Ionicons name="close-circle" size={20} color="#94A3B8" /></TouchableOpacity> : <Ionicons name="chevron-down" size={20} color="#94A3B8" />}</TouchableOpacity></View>
            </View>
            <View style={styles.row}>
              <View style={{ flex: 1 }}><Text style={styles.label}>Tình trạng HĐ <Text style={styles.required}>*</Text></Text><TouchableOpacity style={styles.inputContainer} onPress={() => openSelect("Tình Trạng Hoạt Động", FUNC_STATUS_OPTIONS, setFunctionalityStatus)}><Text style={functionalityStatus ? styles.inputText : styles.placeholderText} numberOfLines={1}>{functionalityStatus ? getLabel(functionalityStatus, FUNC_STATUS_OPTIONS) : "Chọn..."}</Text>{functionalityStatus ? <TouchableOpacity onPress={() => setFunctionalityStatus("")} style={styles.clearIcon}><Ionicons name="close-circle" size={20} color="#94A3B8" /></TouchableOpacity> : <Ionicons name="chevron-down" size={20} color="#94A3B8" />}</TouchableOpacity></View>
              <View style={{ flex: 1 }}><Text style={styles.label}>Thời gian SD (Năm)<Text style={styles.required}>*</Text></Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} keyboardType="numeric" placeholder="VD: 2" placeholderTextColor="#94A3B8" value={usageDuration} onChangeText={setUsageDuration} /></View></View>
            </View>
          </View>
          ...
          <View style={styles.cardSection}>
            ...
            <View style={styles.row}>
              <View style={{ flex: 1 }}><Text style={styles.label}>Giá mong muốn <Text style={styles.required}>*</Text></Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} keyboardType="numeric" placeholder="VNĐ" placeholderTextColor="#94A3B8" value={basePrice} onChangeText={setBasePrice} /></View></View>
              <View style={{ flex: 1 }}><Text style={styles.label}>Giá lúc mua</Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} keyboardType="numeric" placeholder="VNĐ" placeholderTextColor="#94A3B8" value={originalPrice} onChangeText={setOriginalPrice} /></View></View>
            </View>
            <View style={styles.row}>
              <View style={{ flex: 1 }}><Text style={styles.label}>Số lượng <Text style={styles.required}>*</Text></Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} keyboardType="numeric" placeholder="Nhập SL..." placeholderTextColor="#94A3B8" value={quantity} onChangeText={setQuantity} /></View></View>
              <View style={{ flex: 1 }}><Text style={styles.label}>Thời hạn tin (Ngày)</Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} keyboardType="numeric" placeholder="VD: 30" placeholderTextColor="#94A3B8" value={expiryDays} onChangeText={setExpiryDays} /></View></View>
            </View>
            <Text style={styles.label}>Địa chỉ giao dịch <Text style={styles.required}>*</Text></Text><TouchableOpacity style={styles.inputContainer} onPress={openAddressModal}><Text style={displayAddress ? styles.inputText : styles.placeholderText} numberOfLines={1}>{displayAddress || "Tỉnh/Thành, Quận/Huyện, Số nhà..."}</Text>{displayAddress ? <TouchableOpacity onPress={clearAddress} style={styles.clearIcon}><Ionicons name="close-circle" size={20} color="#94A3B8" /></TouchableOpacity> : <Ionicons name="location-outline" size={20} color="#94A3B8" />}</TouchableOpacity>
            <View style={styles.row}>
              <View style={{ flex: 1 }}><Text style={styles.label}>Vận chuyển</Text><TouchableOpacity style={styles.inputContainer} onPress={() => openSelect("Phương Thức Vận Chuyển", DELIVERY_OPTIONS, setDeliveryMethod)}><Text style={deliveryMethod ? styles.inputText : styles.placeholderText} numberOfLines={1}>{deliveryMethod ? getLabel(deliveryMethod, DELIVERY_OPTIONS) : "Chọn..."}</Text>{deliveryMethod ? <TouchableOpacity onPress={() => setDeliveryMethod("")} style={styles.clearIcon}><Ionicons name="close-circle" size={20} color="#94A3B8" /></TouchableOpacity> : <Ionicons name="chevron-down" size={20} color="#94A3B8" />}</TouchableOpacity></View>
              <View style={{ flex: 1 }}><Text style={styles.label}>Ưu tiên</Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} placeholder="Nhập / Chọn..." placeholderTextColor="#94A3B8" value={priorityLevel} onChangeText={setPriorityLevel} />{priorityLevel && <TouchableOpacity onPress={() => setPriorityLevel("")} style={styles.clearIcon}><Ionicons name="close-circle" size={20} color="#94A3B8" /></TouchableOpacity>}<TouchableOpacity style={{ paddingLeft: 8, marginLeft: 4, borderLeftWidth: 1, borderLeftColor: "#E2E8F0" }} onPress={() => openSelect("Mức Độ Ưu Tiên", PRIORITY_OPTIONS, setPriorityLevel)}><Ionicons name="chevron-down" size={20} color="#94A3B8" /></TouchableOpacity></View></View>
            </View>
          </View>
        </ScrollView>
          ...
          <View style={styles.modalOverlay}>
            <View style={[styles.modalContent, { maxHeight: "90%" }]}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} style={{ marginTop: 8 }}>
                <Text style={styles.label}>Tỉnh / Thành phố <Text style={styles.required}>*</Text></Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} placeholder="VD: TP. Hồ Chí Minh" placeholderTextColor="#94A3B8" value={tempCity} onChangeText={setTempCity} /></View>
                <Text style={styles.label}>Quận / Huyện <Text style={styles.required}>*</Text></Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} placeholder="VD: Quận 1" placeholderTextColor="#94A3B8" value={tempWard} onChangeText={setTempWard} /></View>
                <Text style={styles.label}>Số nhà, Tên đường <Text style={styles.required}>*</Text></Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} placeholder="VD: 123 Đường Lê Lợi" placeholderTextColor="#94A3B8" value={tempStreet} onChangeText={setTempStreet} /></View>
                <TouchableOpacity style={styles.primaryButton} onPress={confirmAddress}><Text style={styles.primaryButtonText}>Xác nhận</Text></TouchableOpacity>
              </ScrollView>
            </View>
          </View>
        </KeyboardAvoidingView>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
          <View style={styles.modalOverlay}>
            <View style={[styles.modalContent, { maxHeight: "90%" }]}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} style={{ marginTop: 8 }}>
                <Text style={styles.label}>Chiều dài (cm)</Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} keyboardType="numeric" placeholder="VD: 120" placeholderTextColor="#94A3B8" value={tempLength} onChangeText={setTempLength} /></View>
                <Text style={styles.label}>Chiều rộng (cm)</Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} keyboardType="numeric" placeholder="VD: 60" placeholderTextColor="#94A3B8" value={tempWidth} onChangeText={setTempWidth} /></View>
                <Text style={styles.label}>Chiều cao (cm)</Text><View style={styles.inputContainer}><TextInput style={[styles.input, Platform.OS === "web" ? ({ outlineStyle: "none" } as any) : null]} keyboardType="numeric" placeholder="VD: 80" placeholderTextColor="#94A3B8" value={tempHeight} onChangeText={setTempHeight} /></View>
                <TouchableOpacity style={styles.primaryButton} onPress={confirmDimensions}><Text style={styles.primaryButtonText}>Xác nhận</Text></TouchableOpacity>
              </ScrollView>
            </View>
          </View>
        </KeyboardAvoidingView>
